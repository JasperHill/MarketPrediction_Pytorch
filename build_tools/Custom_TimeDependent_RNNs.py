################################################################################################
## There is no structural difference between Custom_TimeDependent_RNNs.py and Custom_RNNs.py
## There is only the nominal difference of appending 'TD_' to the function and class names
################################################################################################
...
class LSTM_Op_pass(Function):
    ...
    def forward(ctx, input, c_p, h_p, xOps, hOps):
        h, c, Xs = rnn_passes_cpp.TD_LSTM_Op_forward(input.to(torch.double),
                                                     c_p.to(torch.double),
                                                     h_p.to(torch.double),
    ...
    def backward(ctx, grad_output, grad_c, grad_Xs):
        ...
        grad_input, grad_c_p, grad_h_p, grad_xOps, grad_hOps = rnn_passes_cpp.TD_LSTM_Op_backward(input.to(torch.double),
                                                                                                  Xs.to(torch.double),
                                                                                                  c.to(torch.double),
                                                                                                  c_p.to(torch.double),
                                                                                                  h_p.to(torch.double),
...
# TD_LSTM_Op is an LSTM layer whose kernels are true linear operators rather than Hadamard-like operators
# biases are not used
