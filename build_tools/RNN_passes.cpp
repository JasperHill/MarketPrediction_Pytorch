					     ...
					     torch::Tensor xOps, torch::Tensor hOps)
{
  ...
  //this is an experimental modification to the output gate that allows for unconstrained but still reasonably stable output magnitudes
  //auto output_g = xo*(at::exp(-xo*xo/2));
...
std::vector< torch::Tensor > Categorical_LSTM_Op_forward(torch::Tensor x, torch::Tensor c_p, torch::Tensor h_p, \
							 torch::Tensor xOps, torch::Tensor hOps)
{
  //contracting along the -1 axes of the vector and the operator corresponds to x^† * Op^†
  auto xi = torch::add(at::tensordot(x, xOps[0], {-2, -1}, {-3, -2}), at::tensordot(h_p, hOps[0], {-1}, {-2}));
  auto xf = torch::add(at::tensordot(x, xOps[1], {-2, -1}, {-3, -2}), at::tensordot(h_p, hOps[1], {-1}, {-2}));
  auto xg = torch::add(at::tensordot(x, xOps[2], {-2, -1}, {-3, -2}), at::tensordot(h_p, hOps[2], {-1}, {-2}));
  auto xo = torch::add(at::tensordot(x, xOps[3], {-2, -1}, {-3, -2}), at::tensordot(h_p, hOps[3], {-1}, {-2}));
  ...
  for (n = 0; n < batch_size; n++)
    {
      ...
      //auto dh_dxo = grad_output_n * at::tanh(c_n) * (1 - X_n[3]*X_n[3]) * at::exp(-X_n[3]*X_n[3]/2);
...
std::vector< torch::Tensor > Categorical_LSTM_Op_backward(torch::Tensor x, torch::Tensor X, torch::Tensor c, torch::Tensor c_p, torch::Tensor h_p, \
							   torch::Tensor xOps, torch::Tensor hOps, \
							   torch::Tensor grad_output)
{
  //x is of shape (batch_size, gates, input_channels, input_timesteps)
  ...
  //here y is of shape batch/output_dim
  int64_t output_dim = y_dims[1];

  //transpose Xs to shape (batch_size, gates, output_channels, output_timesteps)
  auto X_t = at::transpose(X, 0, 1);
  auto grad_input = torch::zeros_like(x, options);
  auto grad_xOps = torch::zeros({4, input_channels, input_dim}, options);
  auto grad_hOps = torch::zeros({4, 4}, options);

  //todo: parallelize
#pragma omp parallel default(none) schedule(dynamic)			\
  firstprivate(x, X_t, c, c_p, h_p, batch_size, input_channels, input_dim) \
  shared(grad_input, grad_xOps, grad_hOps)				\
  private(n,i,ii,jj)
  {
    #pragma omp for nowait
  ...
  for (n = 0; n < batch_size; n++)
    {
      ...
      auto dh_dc = grad_output_n * d_sigmoid(c_n) * at::sigmoid(X_n[3]);
      auto dh_dxo = grad_output_n * at::tanh(c_n) * d_sigmoid(X_n[3]);
	  ...
	  for (ii = 0; ii < input_channels; ii++)
	    {
	      for (jj = 0; jj < input_dim; jj++)
		{
		  grad_xOps[3][ii][jj][i] += grad_X_n[3][i] * x_n[ii][jj];
		  grad_xOps[2][ii][jj][i] += grad_X_n[2][i] * x_n[ii][jj];
		  grad_xOps[1][ii][jj][i] += grad_X_n[1][i] * x_n[ii][jj];
		  grad_xOps[0][ii][jj][i] += grad_X_n[0][i] * x_n[ii][jj];
	      
		  grad_input_n[ii][jj] += grad_X_n[3][i] * xOps[3][ii][jj];
		  grad_input_n[ii][jj] += grad_X_n[2][i] * xOps[2][ii][jj];
		  grad_input_n[ii][jj] += grad_X_n[1][i] * xOps[1][ii][jj];
		  grad_input_n[ii][jj] += grad_X_n[0][i] * xOps[0][ii][jj];		      
		}//jj
	    }//ii
      
	  grad_hOps[3][i] += grad_X_n[3][i] * h_p_n[i];
	  grad_hOps[2][i] += grad_X_n[2][i] * h_p_n[i];
	  grad_hOps[1][i] += grad_X_n[1][i] * h_p_n[i];
	  grad_hOps[0][i] += grad_X_n[0][i] * h_p_n[i];
	}//i
    }//n
  }
  //c and h are not trainable parameters so return zero gradients
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
  ...
  m.def("Categorical_LSTM_Op_forward", &Categorical_LSTM_Op_forward, "Categorical_LSTM_Op forward");
  m.def("Categorical_LSTM_Op_backward", &Categorical_LSTM_Op_backward, "Categorical_LSTM_Op backward");
}
